...
const Home = () => {
    ...
    useEffect(() => {
        const selectedDateStr = selectedDate.toISOString().split('T')[0]; // تاریخ فقط به صورت YYYY-MM-DD
        setFilter(dataTemp.filter(item => item.dt_txt.split(' ')[0] === selectedDateStr)); // فیلتر کردن بر اساس تاریخ
